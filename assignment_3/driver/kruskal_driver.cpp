...
void writeResultToFile(ofstream& fout,vector<pair<pair<int,int>,int>>& mstEdges,long long totalWeight,double executionTime){
    ...
    fout<<"Total MST weight: "<<totalWeight<<"\n\n";
...
int main(int argc,char* argv[]){
    ...
    auto result=kruskalMST(V,row_ptr,col_idx,weights);

    auto end=chrono::high_resolution_clock::now();

    double executionTime=chrono::duration<double,milli>(end-start).count();

    string outputFile="./outputs/kruskal_output_"+string(argv[1]);

    ofstream fout(outputFile);

    fout<<"Algorithm: Kruskal's MST\n\n";
